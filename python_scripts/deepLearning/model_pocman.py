...
class Encoder(object):
    ...
    def transform_x(self, x):
        trans_x = np.zeros(self.size_x(), dtype=np.bool)
        if x == STUMP:
            trans_x[-2] = 1
        elif x == PAD:
            trans_x[-1] = 1
...
class Seq2SeqGraph(object):
    def __init__(self,
        ...
        for i in xrange(seq_length):
            self.inputs.append(tf.placeholder(tf.bool, shape=(None, input_length), name="input_{0}".format(i)))
