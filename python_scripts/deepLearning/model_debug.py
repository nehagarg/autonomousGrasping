...
def test(model_name, fileName=None):
    # training parameters
    hidden_units = 128
    num_layers = 2
    ...
    with tf.Session() as sess:
        ...
        #cpp_saver = model.training_graph.cpp_saver('output/version4/')
        #cpp_saver(sess, 800)
...
def plot_action_probs(sampleId, action_probs):
    
    action_prob_t = np.transpose(action_probs)
    plt.figure(sampleId)
    num_steps = len(action_prob_t[0])
    num_subplots = int(math.ceil(float(num_steps-1)/10))
    x_plots = 1
    y_plots = 1
    if num_subplots > 1 :
        y_plots = 2
    ...
    for i in range(0,10):
        if i > 5:
            ...
            line_style = 'dashed'
        for j in range(0,num_subplots):
            plt.subplot(x_plots,y_plots,j+1)
            #print j
            #plt.subplot(3,3,j+1)
            plt.plot(range(j*10,(j*10) + len(action_prob_t[i][j*10:(j+1)*10])), action_prob_t[i][j*10:(j+1)*10],linestyle = line_style, label=repr(i))
    plt.legend()
    return plt

def plot_information(sampleId, action_probs):
    plt.figure(sampleId)
    action_information = [sum(j*math.log(j) for j in i) for i in action_probs]
    #print action_information
    plt.plot(action_information)
    return plt
...
def main():
    ...
    #logfileName = '/home/neha/WORK_FOLDER/phd2013/phdTopic/despot/despot-0.2-server-version/4_objects_obs_prob_change_particles_as_state/deepLearning_different_objects/version1/state_' + repr(i) + '.log'
    #model_name = 'version1/model.ckpt-800'
    ...
    #model_name = 'version3/model.ckpt-2000'
    #logfileName = '/home/neha/WORK_FOLDER/phd2013/phdTopic/despot/despot-0.2-server-version/4_objects_obs_prob_change_particles_as_state/deepLearning_different_objects/version4/probabilistic_actions/state_' + repr(i) + '.log'
    #model_name = 'version4/model.ckpt-800'
    #logfileName = '/home/neha/WORK_FOLDER/phd2013/phdTopic/despot/despot-0.2-server-version/4_objects_obs_prob_change_particles_as_state/deepLearning_different_objects/version2/state_' + repr(i) + '.log'
    #model_name = 'version2/model.ckpt-852'
    logfileName = '/home/neha/WORK_FOLDER/phd2013/phdTopic/despot/despot-0.2-server-version/4_objects_obs_prob_change_particles_as_state/deepLearning_different_objects/version5/state_' + repr(i) + '.log'
    model_name = 'version5/model.ckpt-576'
    ...
    action_probs1 = test(model_name, logfileName)
    #print action_probs1[11:13]
    plt1 = plot_action_probs(i, action_probs1)
    #plt1 = plot_information(i,action_probs1)
    plt1.show()
    
    logfileName = '/home/neha/WORK_FOLDER/phd2013/phdTopic/despot/despot-0.2-server-version/4_objects_obs_prob_change_particles_as_state/deepLearning_different_objects/version2/state_' + repr(i) + '.log'
    #action_probs2 = test(logfileName)
    #plt2 = plot_action_probs(i, action_probs2)
    #plt2.show()
